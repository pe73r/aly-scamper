...
jQuery( function ($) {
	var Sections = {
		...
		_onSectionLoad: function( e ) {
			...
			function _loadCollectionTemplate( t ) {
				...
				Collection.init();
				Collection.setCollectionHeight();
				Site.sliders.init();
				QuickView.init();
			...
			function _loadListCollections( t ) {
				...
				$(window)
					.on( 'load', ListCollections.truncateBlockText(collectionList) )
					.on( 'resize', Reqs.throttle( function() {
	...
	window.Site = {
		...
		header: {
			init: function() {
				...
				scroll = 0,
				heroEnabled = $('.bodyWrap').children().first().hasClass( 'header--full' ),
				alertHeight = $('.siteAlert').outerHeight();

				if (!$('.template-index').length) $header.removeClass('header--no-bg');

				$(window)
					.on( 'resize', Reqs.debounce( checkNavigationOverlapping, 10) )
					.on( 'resize', Reqs.throttle( Site.setCartClosePosition, 10) )
					.on( 'scroll', Reqs.throttle( setHeaderState, 10) );

				setTimeout(checkNavigationOverlapping, 50);

				initPadding();

				$(document).on( 'shopify:section:reorder', initPadding );
				$(document).on( 'shopify:section:reorder', checkNavigationOverlapping );
				...
				function initPadding(){
					if($('body').hasClass('template-index')){
						...
						var heroEnabled = $('.bodyWrap').children().first().hasClass( 'header--full' );
				...
				function getNavigationOverlapping() {
					$('.site-header').addClass('is-standard').removeClass('is-hamburger has-scrolled');
			...
			resetHeader: function() {
				...
				var $transformElements = $( '.site-header, .nav-inner, .siteAlert, .bodyWrap' );
		...
		sliders: {
			...
			setSlidesHeight: function(slider){
				...
				slider.find( '.js-slide' ).each(function() {
					var $slide = $(this);
					var isFirstSection = !$slide.closest( '.section-slideshow' ).prev().length;
					var slideContentHeight = $(this).find('.slide-content').outerHeight();
					var additionalPadding = 60;
					var slideMinHeight = slideContentHeight + additionalPadding;

					if ( isFirstSection ) {
						// Add 100px as the height of the header if the section is first to prevent overlapping
						slideMinHeight += 100;
					}

					if ( isImageHeight ) {
						var aspectRatio = isSlideshow ? $slide.find( 'img:visible' ).data( 'aspectratio' ) : $slide.find( 'img' ).data( 'aspectratio' );
						var slideWidth = $slide.width();
						var imageHeight = 0;

						if ( aspectRatio ) {
							imageHeight = parseFloat(slideWidth / aspectRatio).toFixed(3);

							if ( imageHeight < slideMinHeight ) {
					...
					// Set min height to the slide and the images
					$slide.find( '.slide-image img, .slide-image--blank' ).css( 'min-height', slideMinHeight );
					$slide.css( 'min-height', slideMinHeight );
				});
			...
			setCarouselState: function($container) {
				...
				if ( $carousel.length ) {
					...
					var isMobileInitialized = $carousel.data( 'flickity' ) != undefined;
					var slidesPerRow = parseInt( $container.data( 'slides-per-row' ));
					var windowWidth = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;

					if ( windowWidth >= 768 ) {
						// Destroy mobile carousel if initialized
						if ( isMobileInitialized ) {
							$carousel.flickity('destroy');

							// Give time for the mobile slider to be destroyed
							setTimeout( initDesktopCarousel, 100);
						} else {
							initDesktopCarousel();
						}
					} else {
						// Destroy desktop carousel if initialized
						if ( isDesktopInitialized ) {
							$carousel.slick( 'unslick' );

							// Give time for the desktop slider to be destroyed
				...
				function initMobileCarousel() {
					if ( !isMobileInitialized) {
						$carousel.flickity({
							cellSelector: '.js-collectionBlock',
							prevNextButtons: false,
							pageDots: false,
		...
		nav: {
			...
			init: function(){
				...
				$elementsToShift = $('.bodyWrap, .site-header, .siteAlert');
			...
			bindings: function(){
				...
				// A11y
				$('.nav-standard .menu a[aria-expanded]').on( 'focus', function() {
					// Close other mega menus
					$(this).parent().siblings('.menu-item--meganav').removeClass('is-expanded').find('a[aria-expanded]').attr('aria-expanded', false);
					$(this).parent().siblings('.menu-item--meganav').find('.visible-nav-link').attr('tabindex', '-1');

					// Open selected mega menu
					$(this).attr('aria-expanded', true).parent().addClass('is-expanded');
					$(this).parent().find('.visible-nav-link').removeAttr('tabindex');
				});

				// Header state when megamenu is open
				$('.site-header a').on( 'focus', function() {
					if ( $(this).parents('.menu-item--meganav').length ) {
						$('.site-header').addClass('header--megamenu-visible');
					} else {
						// Close mega menu
						$('.visible-nav-link').attr('tabindex', '-1');
			...
			show: function(){
				...
				$elementsToShift.css({ 'transform': 'translateX(' + navMenuWidth + ')' });
			...
			hide: function(){
				...
				$elementsToShift.css({ 'transform': 'none' });

				$(window).off('resize.siteNav');

				setTimeout(function(){
					$body.add('html').removeClass('nav--is-visible nav--is-hiding');
				}, 300);
			...
			submenu: {
				...
				close: function(el){
					...
					$elSubMenu.find('.visible-nav-link').attr('tabindex', '2');
				}
			},
		...
		images: {
			loadBackgrounds: function() {
				...
				$elementsToLoad.each(function() {
					...
					if(src == '') {
						src = '//cdn.shopify.com/s/files/1/0286/8720/4400/t/2/assets/placeholder-pattern.png?v=18306360016701120725';
						placeholder = true;
					}
					...
					$(im).on('error', function() {
						$el.css('background-image', 'url(//cdn.shopify.com/s/files/1/0286/8720/4400/t/2/assets/placeholder-pattern.png?v=18306360016701120725)')
		...
		animations: {
			init: function() {
				...
				if ( enableAnimations ) {
					...
					this.animateAll();
					$(window).on('scroll', Reqs.throttle(self.animateAll, 100));
					$(window).on('load', self.animateAll);
					$(document).on('ajaxify:updated', self.animateAll);
					$(document).on('scroll', '.blogModule-posts', Reqs.throttle(self.animateAll, 100));
				}
			},
	...
	Cart = {
				...
				error: function(data){
					$.ajax({
						...
						success: function(product){
							...
							el.html("Oops"); // swap button text
							setTimeout(function(){
								el.html("Ajouter au panier"); // swap it back
							}, 1000);
		...
		buildCart: function (cart) {
			...
			if ( !$body.hasClass( 'template-cart') ) {
				$cart.append( '<div class="js-cartToggle js-cartToggle-close cart-close icon-close"></div>' );
			}
	...
	var Collection = {
		init: function() {
			...
			$(window)
				.on('load', self.setCollectionHeight)
				.on('resize', Reqs.debounce( self.setCollectionHeight, 250));
		...
		setCollectionHeight: function() {
			var $banner = $( '.banner--collections' );
			var contentHeight = $banner.find( '.page-title > .container' ).outerHeight() + 40;
			var headerHeight = 100;
			var isImageHeight = $banner.find( '.banner-img--original' ).length;

			if ( isImageHeight ) {
				$banner.find( '.banner-img--original' ).css( 'min-height', contentHeight + headerHeight );
			}
		},
	...
	var ListCollections = {
		...
		truncateBlockText: function(collectionList) {
			var fourPerRow = collectionList.find('.collectionBlock').hasClass( 'lg_s14' );
			var textLines = fourPerRow ? 2 : 3;
			...
			$.getJSON(
				...
				function(product) {
					var id = product.id, // int
						...
						compare_at_price_formatted = Shopify.formatMoney(compare_at_price, Shopify.money_format),
						price_formatted = Shopify.formatMoney(price, Shopify.money_format); // string

					self.ajaxed = true; // set ajaxed variable to true, means that ajax has occurred

					if ( showDescription ) {
						description = '<div class="product-container"><div class="product-description rte">' + description + '</div></div>';
					} else {
						description = '';
					}
					...
					for ( i = 0; i < options.length; i++ ) {
						...
						for(i = 0; i < variants.length; i++){
							if (variants[i]['available'] == true && variant_avail == false){
								...
								first_avail_variant_object = variants[i];
							} else {
								selected = '';
							}
					...
					var unitPrice = '';
					if ( first_avail_variant_object.unit_price_measurement ) {
				      unitPrice += '<span class="product-unit-price">';
				      unitPrice += Shopify.formatMoney(first_avail_variant_object.unit_price, Shopify.money_format) + ' ';
				      if ( first_avail_variant_object.unit_price_measurement.reference_value != 1 ) {
				        unitPrice += first_avail_variant_object.unit_price_measurement.reference_value;
				      }
				      unitPrice += first_avail_variant_object.unit_price_measurement.reference_unit;
				      unitPrice += '</span>';
				    }
					...
					if ( enableReviews ) {
						reviews = '<div class="product__badge">' + $collectionBlock.find( '.reviews-fake' ).html() + '</div>';
					...
					if ( showQuantity ) {
						quantitySelect = '<div class="qty__adjust">' +
													...
													'<div class="product-info">' +
														'<div class="product-info__wrapper">' +
															'<div class="product-title__wrapper block s1 ">'+
																'<h2 class="product-title"><a class="js-productUrl" href="' + url + '">' + title + '</a></h2>' +
																'<div class="product-price" data-price="' + price + '">' +
																	pricing +
																	unitPrice +
																'</div>' +
																reviews +
															...
															'<div class="product-form block s1">' +
																'<form class="productForm" action="/cart/add" method="post" data-color_swatches="' + enableColorSwatches + '" data-product_id="' + id + '">' +
																	'<div class="productForm-block productForm-block--options-inline ' + productBlockSwatchesClass + '">' +
																		swatches +
																		dropdowns +
																	'</div>' +
																	'<div class="productForm-block">' +
																		quantitySelect +
																		'<div><button class="js-productForm-submit productForm-submit" type="submit" data-handle="' + product_handle + '" data-ajax="true">' + "Ajouter au panier" + '</button></div>' +
																	'</div>' +
																'</form>' +
																description +
															'</div>' +
		...
		selectOptions: function(el, obj){
			...
			function callback(options){
				...
				var $submit = el.find('.js-productForm-submit'),
					...
					$unit_price = el.find('.product-unit-price'),
					$counter = el.find('.js-counter').not('.cart-product-quantity .js-counter'),
					$sale_container = $price.find('.sale');

				if (variant) {
					if (variant.available) {
						$submit.removeClass('is-disabled').prop('disabled', false).html("Ajouter au panier");
						...
						if (variant.compare_at_price != null){
							if (variant.compare_at_price > variant.price) {
								if ($sale_container.length){
									...
									$('.product-compare-price').html(Shopify.formatMoney(variant.compare_at_price, Shopify.money_format));
									$('.product-sale-price').html(Shopify.formatMoney(variant.price, Shopify.money_format));
								}
								$normal_price.hide();
								$sale_container.show();
					...
					// this variant sold out
						...
						$submit.addClass('is-disabled').prop('disabled', true).html("Vendu");
					...
					$submit.addClass('is-disabled').prop('disabled', true).html("Indisponible");
...
var Product = {
	...
	initScroller: function( update ) {
		...
		if (productImages > 1) {
			...
			$(window)
				.on('scroll', Reqs.throttle(stickyThumbnails, 10) )
				.on('load', function() {
					isPageLoaded = true;
				});
			...
			$(window).on('resize', Reqs.throttle( function(){
				toggleMobileSlider();
	...
	callback: function(options){
		...
		if (variant) {
			if (variant.available) {
				$submit.removeClass( 'is-disabled' ).prop( 'disabled', false ).html("Ajouter au panier");
				$shopbar_submit.removeClass('is-disabled').html("Acheter");
				$counter.css({'opacity': 1, 'pointer-events': 'auto'});
				$price.css({'opacity': 1});
			...
			// this variant sold out
				...
				$submit.addClass('is-disabled').prop('disabled', true).html("Épuisé");
				$shopbar_submit.addClass('is-disabled').html("Épuisé");
				$counter.css({'opacity': 0.3, 'pointer-events': 'none'});
				$price.css({'opacity': 0.3});
	...
	qtyAdjust: function() {
		...
		$body.on( 'click', '.js-qty__adjust-btn', function() {
			var $adjustBtn = $(this),
...
var Blog = {
	init: function() {
		...
		self.mobileCheck();
	    $(window)
			.on( 'load', self.truncateText )
			.on( 'resize', Reqs.throttle( self.truncateText, 50 ))
	    	.on( 'resize', Reqs.debounce( self.mobileCheck.bind( self ), 150 ));

		if (showFeatured) {
			self.truncateFeaturedText();
			$(window)
				.on('load', self.truncateFeaturedText)
				.on('resize', Reqs.throttle( self.truncateFeaturedText, 50));

			// Start from second page if featured article enabled
			$blog.find('.loadMore').trigger('click');
		}

		$(document).on('ajaxify:updated', self.truncateText);
	},

	truncateText: function() {
		var itemSelector = $('.blogModule-posts__post');

		if ( itemSelector.length ) {
			itemSelector.each(function() {
				var itemContent = $(this).find('.post-title, .excerpt');
				var hasImage = $(this).hasClass( 'blogModule-posts__post--has-image' );
				var lines = hasImage ? 2 : 4;
	...
	mobileCheck: function() {
		window.innerWidth < 768 ? this.initSlider() : this.destroySlider();
	},

	initSlider: function() {
		var self = this;
		$( '.blogModule-posts--slider-mobile' ).each( function() {
			var $slider = $(this);

			$slider.on( 'ready.flickity', function() {
				$(this).find( '.blogModule-posts__post' ).removeClass( 'will-animate' );
				setTimeout( function() {
					self.truncateText();
			...
			if ( !$slider.data('flickity') ) {
				$slider.flickity({
					cellSelector: '.blogModule-posts__post',
					contain: true,
					freeScroll: true,
					prevNextButtons: false,
					pageDots: false,
		...
		if( $('.js-instafeed').length ) {
			$('.js-instafeed').each(function() {
				...
				if (enableFeaturedImage){
					featuredClass = ' instagram-img__wrapper--featured';
				}
				...
				var template = '<div class="instagram-img__wrapper%%featuredClass%%">\
									<a class="instagram-img lazyload" data-bgset=%%img%% data-sizes="auto" data-parent-fit="cover" target="_blank" href="%%link%%">\
						...
						$.get( 'https://api.instagram.com/v1/users/self?access_token=' + token, function( data ) {
							...
							var feed = new Instafeed({
						        ...
						        success: function( data ) {
									...
									instaFeed.find('.instagram-img__content').append( '<div class="instagram-img__btn">Voir sur Instagram</div>' );

									var truncateCaption = function() {
										var lines = 3;
...
var Search = {
	init: function() {
		var self = this;
		self.searchForm = $('.nav-search .search-form');
		self.searchContainer = $('.nav-search-container');
		self.searchScroller = $('.nav-search-scroller');
	...
	request: function(query) {
		...
		self.searchRequest = $.ajax({
			...
			if (data.results_count) {
				var resultsHeading = "Produits";
				var nextPage = data.next_page;

				searchResultsContent += '<div class="results__heading">' + resultsHeading + '</div>\
										 <ul class="results__list">' + self.getResultsContent(data) + '</ul>';

				if (nextPage) {
					pagination = '<div class="pagination u-center"><a href="' + nextPage + '" class="js-loadMore loadMore loadMore--endlessScroll button">Charger plus</a><div class="load-more-icon"></div></div>';
				}

			} else {
				searchResultsContent = '<p class="no-results">Aucun résultat trouvé</p>';
			}
	...
	infiniteScroll: function() {
		...
		self.searchScroller.on('scroll', Reqs.debounce(function() {
			var scrolled = $(this).scrollTop();
